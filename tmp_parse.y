...
BREAK_SYM RETURN_SYM SWITCH_SYM CASE_SYM DEFAULT_SYM LSHIFT RSHIFT
%%
program 
: translation_unit
...
;
compound_statement
: LR {$$=current_id;current_level++;} RR {checkForwardReference(); $$=makeNode(N_STMT_COMPOUND,NIL,NIL,makeNode(N_STMT_LIST,$1,NIL,makeNode(N_STMT_LIST_NIL,NIL,NIL,NIL))); current_id=$2; current_level--;}
| LR {$$=current_id;current_level++;} compound_complex_list RR {checkForwardReference(); $$=$3; current_id=$2; current_level--;}
;
compound_complex_list
: compound_complex {$$=$1;}
| compound_complex_list declaration {$$=linkDeclarationToComplex($1,$2);}
| compound_complex_list statement {$$=makeNodeList(N_STMT_LIST,$1,$2);}
;
compound_complex
: declaration {$$=makeNode(N_STMT_COMPOUND,$1,NIL,makeNode(N_STMT_LIST,$1,NIL,makeNode(N_STMT_LIST_NIL,NIL,NIL,NIL)));}
| statement {$$=makeNode(N_STMT_COMPOUND,NIL,NIL,$1);}
;
...
| shift_expression LSHIFT additive_expression {$$=makeNode(N_EXP_LSHIFT,$1,NIL,$3);}
| shift_expression RSHIFT additive_expression {$$=makeNode(N_EXP_RSHIFT,$1,NIL,$3);}
;
additive_expression
