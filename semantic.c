...
A_TYPE *sem_expression(A_NODE *node)
{
	...
	switch(node->name) { 
		...
		case N_EXP_ADD :
			t1 = sem_expression(node->llink);
			t2 = sem_expression(node->rlink);
			if(isIntegralType(t1) && isArithmeticType(t2)){
				result=convertUsualBinaryConversion(node);
			}
		...
		case N_EXP_SUB :
			...
			else if((isPointerType(t1) && isIntegralType(t2)) || (isCompatiblePointerType(t1, t2))){
				result = t1;
			}
			else{
				semantic_error(24, node->line);
		...
		case N_EXP_EQL :
			...
			else if(!isCompatiblePointerType(t1,t2) && (!isPointerType(t1) || !isConstantZeroExp(node->llink)) && (!isPointerType(t2) || !isConstantZeroExp(node->rlink))){
				semantic_error(40, node->line);
			}
			result = int_type;
			break;
	...
	switch (id->kind) {
		case ID_VAR:
			...
			if(id->level == 0){
				...
				global_address = global_address + i;
			}
			else{
				id->address = addr;
				size = i;
