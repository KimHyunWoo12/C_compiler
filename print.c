...
void prt_program(A_NODE *node, int s)
{
	...
	switch(node->name) { 
		case N_PROGRAM:
			...
			break;
		default :
		printf("****syntax tree error in program******");
	}
}
void prt_initializer(A_NODE *node, int s)
{
	...
	switch(node->name) { 
		...
		case N_INIT_LIST_NIL:
			break;
		default :
			printf("****syntax tree error in initializer******");
	}
}
void prt_expression(A_NODE *node, int s)
{
	...
	switch(node->name) { 
		...
		case N_EXP_RSHIFT :
			...
			break;
		default : 
			printf("****syntax tree error in expression******");
	}
}
void prt_arg_expr_list(A_NODE *node, int s)
{
	...
	switch(node->name) { 
		...
		case N_ARG_LIST_NIL : 
			break; default :
			printf("****syntax tree error in expr list******");
	}
}
	...
	switch(node->name) {
		...
		case N_STMT_RETURN:
			if(node->clink) 
				prt_expression(node->clink, s+1);
		break; default :
			printf("****syntax tree error in statement******");
	}
}
void prt_statement_list(A_NODE *node, int s)
{
	...
	switch(node->name) {
		...
		case N_STMT_LIST_NIL:
			break;
		default :
			printf("****syntax tree error in statementlist******");
	}
}
	...
	switch(node->name) {
		case N_FOR_EXP :
			...
			if(node->rlink) 
				prt_expression(node->rlink, s+1);
			break; default :
				printf("****syntax tree error in for expression******");
	}
}
